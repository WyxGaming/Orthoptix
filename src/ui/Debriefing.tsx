...
const STYLE_LIGNE: Record<LigneScore['nature'], string> = {
  acquis: 'border-accent/30 bg-sage-soft',
  bonus: 'border-accent/30 bg-accent-soft/50',
  manque: 'border-honey-soft bg-honey-soft',
  malus: 'border-blush bg-blush-soft',
};
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 p-6">
      <header className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-ink">Debriefing — {cas.titre}</h1>
          <p className="text-ink-muted">
            {cas.patient.prenom}, {cas.patient.age} ans.
          </p>
        </div>
        <div className="text-right">
          <div className="text-4xl font-semibold text-accent-deep">
            {resultat.total}
            <span className="text-lg text-ink-muted"> / {resultat.max}</span>
          </div>
          <div className="text-sm text-ink-muted">{resultat.pourcentage} % du bilan attendu</div>
        </div>
      </header>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
                    ...
                    {lignes.map((ligne, i) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <span className="text-sm text-ink/90">{ligne.libelle}</span>
                        ...
                        {ligne.commentaire && (
                          <p className="mt-1 text-xs text-ink-muted">{ligne.commentaire}</p>
                        )}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="space-y-5">
          <Carte titre="Compte rendu de l'expert">
            <ul className="space-y-2 text-sm text-ink/80">
              {cas.compteRenduExpert.map((ligne, i) => (
                <li key={i} className="border-l-2 border-accent/40 pl-2.5">
                  {ligne}
                </li>
              ))}
            </ul>
          ...
          >
            {bilan.length === 0 ? (
              <p className="text-sm text-ink-muted">Aucun élément consigné.</p>
            ) : (
              <ul className="space-y-2 text-sm">
                {bilan.map((ligne) => (
                  <li key={ligne.id} className="border-l-2 border-accent/40 pl-2.5">
                    <span className="text-xs font-medium text-accent-deep">{ligne.titre} — </span>
                    <span className="text-ink/80">{ligne.contenu}</span>
                  </li>
                ))}
              </ul>
            )}
